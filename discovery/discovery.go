...
func InitNacosDiscoveryClient(nacosDiscoveryConfig NacosDiscoveryConf) {
	clientConfig := constant.ClientConfig{
		NamespaceId:         nacosDiscoveryConfig.Namespace,
		TimeoutMs:           50000,
		NotLoadCacheAtStart: true,
		Username:            nacosDiscoveryConfig.Username,
		Password:            nacosDiscoveryConfig.Password,
	}
	serverConfig := []constant.ServerConfig{
		{
			IpAddr: nacosDiscoveryConfig.Ip,
			Port:   uint64(nacosDiscoveryConfig.Port),
		},
	}
	...
	registerService(nacosClient, nacosDiscoveryConfig)
}
func registerService(namingClient naming_client.INamingClient, nacosDiscoveryConfig NacosDiscoveryConf) {
	// 定义服务实例
